...
def init(state):
    query_num = input("Which query would you like to try [1/2]?   ")
    while True:
        if (query_num == "1") or (query_num == "2"):
            break
        query_num = input("\nInvalid query number. Please try again [1/2].   ")

    # Read input file
    with open(f'queries/query_input{query_num}.txt', 'r') as file:
    ...
    with open(f'queries/resources/{state['query_name']}.txt', 'r') as file:
...
def generate_pq(state):
    prompt_info = {
        'data_file': f'queries/resources/{state["data_file"]}',
        'columns': state['columns'],
        'description': state['description'],
        'query_output': f'{state["query_name"]}_output',
    }
...
def check_4_err(state):
    ...
    state['errors'] = ""
    # If errors occured during execution
    if (state['exe_output'] != ""):
        state['errors'] = state['exe_output']
    # If output file was not created
    elif not (os.path.exists(f'{state["query_name"]}_output.txt')):
        state['errors'] = "The program executed without errors but the output file was not created."
    else:
        with open(f'{state["query_name"]}_output.txt', 'r') as file:
